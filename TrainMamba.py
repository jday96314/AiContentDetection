...
def SetupDataLoaders(persuade_sampling_proportion, tokenizer_name, batch_size, max_sequence_length, training_example_count, cross_validation_fold_id):
    ...
    FIXED_ORDER_DATA_AUGMENTATION_STEPS = [
        {'name' : 'BuggySpellCheck', 'p' : [0.7, 0.2, 0.2, 0.2, 0.2] },
        {'name' : 'RemoveBlacklistedCharacters', 'p' : [0.7, 0.2, 0.2, 0.2, 0.2] },
    ]
    ...
    training_persuade_dataset = PersuadeDataset(
        ...
        whitelisted_assignment_names = PersuadeDataset.COMPETITION_ASSIGNMENTS + PersuadeDataset.OTHER_ASSIGNMENTS,
    )
    ...
    total_completion_count = len(training_pile_dataset) + (len(training_pajama_dataset) // 2)
    completions_sampling_ratio_ratios = np.array([len(training_pile_dataset), len(training_pajama_dataset)//2]) / total_completion_count
    CRAWL_SAMPLING_PROPORTION = 0.02
...
def TrainModel(persuade_sampling_proportion, max_learning_rate, label_smoothing, training_example_count, output_subdir):
    # SETUP DATALOADER.
    ...
    model.load_state_dict(torch.load('Models/Mamba/3090_1/S12500_CTX1024_10_991_992.pth'))
    model = model.cuda()
        ...
        if (step_number % train_steps_per_test == 0) or (step_number == total_step_count):
            ...
            if (average_auroc > best_auroc) or (max(auroc_scores_by_dataset) > 0.993):
                ...
                if output_subdir is not None:
                    torch.save(model.state_dict(), f'Models/Mamba/{output_subdir}/S{step_number}_CTX{MAX_SEQUENCE_LENGTH_TOKENS}_{int(persuade_sampling_proportion * 1000)}_{int(auroc_scores_by_dataset[0] * 1000)}_{int(auroc_scores_by_dataset[1] * 1000)}.pth')

            train_losses = []

    return best_auroc

def RunExperiments(max_learning_rate, label_smoothing, output_subdir):
    aurocs = []
    for trial_id in range(1):
        auroc = TrainModel(
            ...
            training_example_count = 2 * 50000,
...
if __name__ == '__main__':
    ...
    for lr in [5e-7]:
        ...
        min_auroc, mean_auroc, max_auroc = RunExperiments(lr, label_smoothing, output_subdir)
